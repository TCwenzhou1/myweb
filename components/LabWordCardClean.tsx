...
function getExample(item: VocabEntry) {
  ...
  return {
    ja: `${item.word} を文脈の中で覚えると、長く残りやすいです。`,
    zh: '把这个词放进语境里记，会比只背释义更牢。',
  }
}
...
function getNotes(item: VocabEntry, isFavorite: boolean, inReview: boolean, dueToday: boolean) {
  ...
  if (dueToday) return ['这张词卡今天已经到期，建议先听一遍发音，再做一轮完整复习。']
  if (inReview) return ['这张词卡已经进入复习队列，今天可以顺手给一次难度反馈。']
  if (isFavorite) return ['你已经把它放进收藏本了，下一步建议加入复习，让记忆真正滚起来。']
  return ['先收藏，再加入复习队列，会比只看一遍更容易形成稳定记忆。']
}

export function LabWordCardClean({
  item,
  isFavorite,
  inReview,
  dueToday,
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                isFavorite ? 'bg-[#201911] text-[#fff6e9]' : 'border border-[#dfcfb7] bg-white text-[#5b4630] hover:border-[#c9af84] hover:bg-[#fff7eb]'
              }`}
            >
              {isFavorite ? '已收藏' : '收藏'}
            </button>
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                inReview ? 'bg-[#ffe6d7] text-[#a24d1a]' : 'border border-[#dfcfb7] bg-white text-[#5b4630] hover:border-[#c9af84] hover:bg-[#fff7eb]'
              }`}
            >
              {inReview ? '移出复习' : '加入复习'}
            </button>
