...
export const projectCases: ProjectCase[] = [
  ...
  },
  {
    ...
    title: 'AI 公司指挥工作台',
    headline: '把组织职责、事项升级、员工电脑和领导对话放进同一个 AI 公司指挥工作台。',
    description:
      '这是一个面向 AI 工作流和组织协作的交互原型。它用老板指挥台、链路监控、员工电脑、任务计划和领导对话，把“事情如何在组织里流转”做成可以直接体验的网页 demo。',
    tags: ['Vite', 'React', 'Zustand', 'LLM Agents', 'Vercel Functions'],
    status: 'active',
    year: '2026',
    period: '2026',
    ...
    background:
      '很多 AI agent 展示只停留在聊天框或任务列表里，很难让人直观看到组织边界、上报链路和不同角色之间的协作关系。这个原型把公司运行过程整理成指挥台，让老板视角、员工视角和任务链路能在同一个界面里切换。',
    problem:
      '如果只展示一个 agent 回复，访客看不到任务来源、阻塞原因和决策链路；如果只做普通 dashboard，又缺少沉浸感和可记忆的体验。核心问题是如何把组织流程、AI 对话和可视化交互合成一个清楚的产品原型。',
    solution:
      '方案用 Command OS 承载公司运行总览，用链路监控展示任务与事件状态，用员工电脑承载交付物和任务进度，用领导对话承载 AI 分析。模型调用通过站内服务端代理，浏览器端不保存 API Key。',
    architecture: [
      'React 负责老板指挥台、任务流转、员工电脑和对话面板',
      'Zustand 负责员工、任务、事件、对话、模拟时间和 LLM 状态',
      'Vercel API Route 作为模型代理，浏览器只请求站内接口，不保存 API Key',
      '项目详情页和可交互 demo 集成到个人站 Projects 板块',
    ],
    challenges: [
      '工作台信息密度较高，需要让任务、事件、员工状态和模型状态保持清晰',
      'AI Key 不能留在 localStorage 或浏览器请求里，必须迁移到服务端环境变量',
      '事件生成和任务流转需要面对空数据和未来扩展，不能只依赖当前静态数据正确',
    ],
    outcomes: [
      '形成了可公开访问的组织 agent 指挥工作台，可以从 Projects 详情页直接打开',
      '最新版本移除了旧版沉浸场景入口，改为更直接的老板工作台和链路监控界面',
      '新增回归测试覆盖 API 设置持久化和事件生成防崩溃逻辑',
    ],
    contributions: [
      '完成老板指挥台、链路监控、员工电脑和领导对话的整体产品结构',
      '把模型调用改造成服务端代理，避免公开网页泄露密钥',
